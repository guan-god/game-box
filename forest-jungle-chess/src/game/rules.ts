...
const hasTaunt=(b:Piece[],side:'red'|'blue')=>b.some(p=>p.alive&&p.side===side&&(p.type==='dog'||p.type==='lion'));
const trappedRank=(d:Piece)=> (d.side==='red'&&TRAPS.blue.some(([r,c])=>r===d.row&&c===d.col))||(d.side==='blue'&&TRAPS.red.some(([r,c])=>r===d.row&&c===d.col))?0:d.rank;
export const canCapture=(board:Piece[],a:Piece,d:Piece,g:GameState)=>{if(a.side===d.side)return false; if(d.stealth&&!d.hasAttacked) return false; if(a.type!=='rat'&&isRiverCell(d.row,d.col))return false; if(a.type==='rat'&&isRiverCell(a.row,a.col)&&!isRiverCell(d.row,d.col)&&d.type==='elephant')return false; if(a.type==='rat'&&d.type==='elephant')return true; if(a.type==='elephant'&&d.type==='rat'&&!g.config.elephantCanEatRat)return false; return a.rank>=trappedRank(d);};
...
export const canMoveTo=(board:Piece[],p:Piece,t:[number,number],g:GameState)=>{const [r,c]=t;if(!inb(r,c)||isOwnDen(p,r,c))return false; if(isEnemyDen(p,r,c)&&hasTaunt(board,p.side==='red'?'blue':'red')) return false; const occ=at(board,r,c); if(occ?.side===p.side)return false; const dr=Math.abs(r-p.row),dc=Math.abs(c-p.col); const one=dr+dc===1;
if(!one){
...
return out;};
export const applyMove=(board:Piece[],m:Move,g:GameState)=>{const nb=board.map(p=>({...p}));const p=nb.find(x=>x.id===m.pieceId)!; if(m.captureId){const d=nb.find(x=>x.id===m.captureId)!; if(d.shield){d.shield=false; g.noCapture++; g.moves.push(m); g.turn=g.turn==='red'?'blue':'red'; return nb;} d.alive=false; g.captured[d.side].push(d); g.noCapture=0; if(d.type==='cat'||d.type==='rat'){p.rank=Math.max(1,p.rank-1);} p.hasAttacked=true; } else g.noCapture++; p.row=m.to[0];p.col=m.to[1];g.moves.push(m); const md=Math.abs(m.to[0]-m.from[0])+Math.abs(m.to[1]-m.from[1]); if(md>1||Math.abs(m.to[0]-m.from[0])===1&&Math.abs(m.to[1]-m.from[1])===1) g.powerUsed.push(p.id); if(p.combo&&m.captureId&&g.comboTurn!==p.side){g.comboTurn=p.side;g.turn=p.side;} else {g.comboTurn=undefined; g.turn=g.turn==='red'?'blue':'red';} return nb;};
